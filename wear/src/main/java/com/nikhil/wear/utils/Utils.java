...
public class Utils {
    ...
    public static void vibrate(Context context, int duration) {
        if (duration == 0) {
            duration = DEFAULT_VIBRATION_DURATION_MS; //is neglected in below code.
        }
        ...
        if (Build.VERSION.SDK_INT < 26){
            /*AudioAttributes audioAttributes = new AudioAttributes.Builder()
                    ...
                    .build();
            v.vibrate(DEFAULT_VIBRATION_DURATION_MS, audioAttributes);*/
            v.vibrate(duration);
        } else {
            VibrationEffect effect = VibrationEffect
                    .createOneShot(DEFAULT_VIBRATION_DURATION_MS, DEFAULT_AMPLITUDE);
            v.vibrate(effect);
        }
